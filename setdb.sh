...
PGPASSWORD='michael6295' psql --host=${HOST} --user=${USER} --port=${PORT} ${DB} -c 'DROP DATABASE airbnb_rds;'
PGPASSWORD='michael6295' psql --host=${HOST} --user=${USER} --port=${PORT} ${DB} -c 'CREATE DATABASE airbnb_rds OWNER leesoo;'
